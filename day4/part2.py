...
#variables to track the minute slept most by a single guard
...
for index in range(len(events_list)):
    ...
    if currentLog.event_type == EventType.SHIFT_STARTS:
        ...
        #check if this guard has slept more during a minute than any others so far, if so mark him as the sleepiest guard
        if max(guard_status[active_guard].minute_status) > max_sleepiest_minute:
            max_sleepiest_minute = max(guard_status[active_guard].minute_status)
